...
def parse_raw_loc_output(raw_output, valid_files):
    ...
    # Prefer content inside triple-backtick code blocks; fall back to full text
    code_blocks = re.findall(r'```(?:\w*\n)?(.*?)```', raw_output, re.DOTALL)
    if code_blocks:
        # Try all code blocks (reverse order) to find one with valid .py file paths
        best_block = None
        for block in reversed(code_blocks):
            if re.search(r'[\w/-]+\.py', block) and not block.strip().startswith(('def ', 'import ', 'class ', 'from ')):
                best_block = block
                break
        raw_output = best_block if best_block else code_blocks[-1]
    else:
        # No code block — use full text
    ...
    # Fallback: if no files found, extract .py paths from the original full text
    if not file_list:
        all_py_paths = re.findall(r'[\w\./-]+\.py', original_text)
        for match in all_py_paths:
            fn = extract_python_file_path(match, valid_top_folder)
            if fn and fn in valid_files and fn not in file_list:
                file_list.append(fn)
