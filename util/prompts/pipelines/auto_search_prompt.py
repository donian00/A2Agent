...
You have 7 tools and a `finish` action. Choose the most appropriate tool for each step.

### Search Tools (discover and find)
| Tool | When to Use |
|------|-------------|
| `explore_tree_structure` | Understand repo layout and module hierarchy. |
| `search_summary` | Semantic file discovery: search all file summaries by concept or description (e.g., "validation logic", "serialization"). Returns top matching files with their summaries. Use this when the issue is vague, uses domain-level language, or you don't know exact identifiers. |
...
| `search_commit` | Find relevant commits: search past commits by keyword. Returns commit SHAs, messages, dates, and changed files. Use this to find co-changed files, past fixes, and modification patterns. |

### View Tools (inspect in detail)
| Tool | When to Use |
|------|-------------|
| `get_entity_contents` | Read actual code: once you know the file/class/function path, read its implementation to understand the logic. |
| `view_summary` | Understand a file's purpose: view the LLM-generated summary of a specific file. |

### Finish
| Tool | When to Use |
|------|-------------|
| `finish` | Submit final answer: call this when you are confident you have localized all relevant locations. |

**IMPORTANT**: Use the right tool for the right purpose. Each tool provides a different perspective — choose based on what information you need, not on a fixed order. Call `finish` when you are confident in your answer.
