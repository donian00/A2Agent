...
from .memory_tools import SearchCommitTool, ViewSummaryTool, SearchSummaryTool
import logging
logger = logging.getLogger()

ALL_FUNCTIONS = [
    'explore_tree_structure', 'search_code_snippets', 'get_entity_contents',
    'search_commit', 'view_summary', 'search_summary',
]
...
def get_tools(
    ...
    tools = [FinishTool]
    if codeact_enable_search_keyword and 'search_code_snippets' not in exclude:
    ...
    if codeact_enable_file_summary:
        if 'view_summary' not in exclude:
            tools.append(ViewSummaryTool)
        if 'search_summary' not in exclude:
            tools.append(SearchSummaryTool)
    return tools
