...
class CodeActActionParserFinish:
    def check_condition(self, action_str: str) -> bool:
        # 1) Original <finish>...</finish> tag
        self.finish_command = re.search(r'<finish>.*</finish>', action_str, re.DOTALL)
        if self.finish_command is not None:
            return True
        # 2) Fallback: SFT-trained models output finish as a code block with file paths
        #    Detect if the message contains a ``` block with file paths and NO tool calls
        if '<function=' not in action_str and '<execute_ipython>' not in action_str:
            code_block = re.search(r'```\n(.*?)\n```', action_str, re.DOTALL)
            if code_block and re.search(r'\w+/[\w/.-]+\.py', code_block.group(1)):
                self.finish_command = code_block
                self._is_codeblock_finish = True
                return True
        self._is_codeblock_finish = False
        return False
    ...
    def parse(self, action_str: str) -> Action:
        ...
        if getattr(self, '_is_codeblock_finish', False):
            # For code-block style finish, the whole message is the thought
            thought = action_str
        else:
