...
def main():
    ...
    # For multimodal models (e.g. Qwen3.5), the unsloth tokenizer is a processor
    # that tries to parse text as images. Load a text-only tokenizer separately.
    is_multimodal = hasattr(tokenizer, 'tokenizer')
    if is_multimodal:
        text_tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True)
    ...
    dataset = dataset.map(tokenize_func, batched=True, num_proc=4,
    ...
    trainer = SFTTrainer(
        ...
        max_seq_length=args.max_seq_length,
        max_length=args.max_seq_length,
        data_collator=DataCollatorForSeq2Seq(tokenizer=text_tokenizer),
        dataset_num_proc=4,
        packing=False,
